...
export default function AppSidebar({ collapsed, onToggle }: { collapsed: boolean, onToggle: () => void }) {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 z-30 flex h-screen flex-col border-r border-sidebar-border bg-sidebar sidebar-transition ${collapsed ? "w-[60px]" : "w-72"}`}
    >
      {/* Logo */}
      <div className={`flex flex-col items-center justify-center border-b border-sidebar-border/50 py-6 sidebar-transition ${collapsed ? "px-2" : "px-4"}`}>
        ...
        <div className={`text-center sidebar-content-fade ${collapsed ? "opacity-0 scale-95 pointer-events-none" : "opacity-100 scale-100"}`}>
      ...
      <nav className="flex-1 overflow-y-auto py-3 space-y-4">
        ...
        <div className="px-2 mb-2">
          <p className={`section-label sidebar-content-fade ${collapsed ? "opacity-0 translate-x-[-10px]" : "opacity-100 translate-x-0"}`}>BIM Engine</p>
          <button
          ...
          >
            ...
            <div className={`flex flex-1 items-center justify-between sidebar-content-fade ${collapsed ? "opacity-0 scale-95" : "opacity-100 scale-100"}`}>
        ...
        {navGroups.map((group) => (
          <div key={group.label}>
            <p className={`section-label sidebar-content-fade ${collapsed ? "opacity-0 translate-x-[-10px]" : "opacity-100 translate-x-0"}`}>{group.label}</p>
            <div className="space-y-0.5 px-2">
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className="h-4 w-4 shrink-0" />
                    <span className={`truncate sidebar-content-fade ${collapsed ? "opacity-0 translate-x-[-10px]" : "opacity-100 translate-x-0"}`}>{item.title}</span>
                  </Link>
                );
              })}
            </div>
